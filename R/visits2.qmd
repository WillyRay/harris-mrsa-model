...
df1 <- read.table("visit_data.txt", header = TRUE, sep = ",", stringsAsFactors = FALSE)
df2 <- df1[df1$patientLocation == 'ICU', ]
...
## Distribution of Number of Unique Patients Visited per Day by HCW Type
...
ggplot(unique_patients_per_nurse_day, aes(x = n_unique_patients)) +
	...
	labs(title = "Distribution of Unique Patients Visited per Nurse per Day",
...
**Min unique patients per Nurse per day:** `r min_nurse_patients`  
**Max unique patients per Nurse per day:** `r max_nurse_patients`  
**Mean unique patients per Nurse per day:** `r mean_nurse_patients`  
**Median unique patients per Nurse per day:** `r median_nurse_patients`

## Unique Patients visited per day
### Doctors
```{r}
...
ggplot(unique_patients_per_doctor_day, aes(x = n_unique_patients)) +
	...
	geom_vline(aes(xintercept = mean(n_unique_patients)), color = "blue", linetype = "dashed", size = 1) +
	geom_vline(aes(xintercept = median(n_unique_patients)), color = "red", linetype = "dotted", size = 1) +
	labs(title = "Distribution of Unique Patients Visited per Doctor per Day",
			 x = "Number of Unique Patients",
			 y = "Count (Doctor-Day pairs)",
			 subtitle = "Blue dashed = mean, Red dotted = median") +
	theme_minimal()
...
**Min unique patients per Doctor per day:** `r min_doctor_patients`  
**Max unique patients per Doctor per day:** `r max_doctor_patients`  
**Mean unique patients per Doctor per day:** `r mean_doctor_patients`  
**Median unique patients per Doctor per day:** `r median_doctor_patients`


## Distribution of Number of Unique Healthcare Providers Seen by a Patient per Day
...
ggplot(unique_providers_per_patient_day, aes(x = n_unique_providers)) +
	...
	labs(title = "Distribution of Unique Healthcare Providers Seen by a Patient per Day",
...
**Min unique providers per patient per day:** `r min_patient_providers`  
**Max unique providers per patient per day:** `r max_patient_providers`  
**Mean unique providers per patient per day:** `r mean_patient_providers`  
**Median unique providers per patient per day:** `r median_patient_providers`
